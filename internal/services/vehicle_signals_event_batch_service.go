		...
		for key, value := range eventAvailableProperties {
			if _, ok := data[key]; ok {
				...
				if err := eventProperties.Upsert(ctx, v.db().Writer, true, reportVehicleSignalsPrimaryKeyColumns, boil.Infer(), boil.Infer()); err != nil {
					v.log.Err(err).Msgf("error inserting report : %s %s %s %s", eventProperties.DateID, eventProperties.IntegrationID, eventProperties.DeviceMakeID, eventProperties.PropertyID)
				}
			}
		}
