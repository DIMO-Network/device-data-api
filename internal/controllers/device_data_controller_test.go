...
func TestUserDevicesController_GetUserDeviceStatusV2(t *testing.T) {
	// arrange global db and route setup
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	...
	c := NewDeviceDataController(&config.Settings{Port: "3000"}, &logger, deviceSvc, nil, deviceDefSvc, deviceStatusSvc, pdb.DBS)
	app := fiber.New()
	app.Get("v2/user/device-data/:userDeviceID/status", test.AuthInjectorTestHandler(testUserID), c.GetUserDeviceStatusV2)
	...
	t.Run("GET - device status", func(t *testing.T) {
		...
		notCharging := false
		var expectedResp response2.Status
		expectedResp.PowerTrain.TractionBattery.Charging.IsCharging = null.BoolFrom(notCharging)
		expectedResp.PowerTrain.Transmission.TravelledDistance = null.Float64From(195677.59375)
		expectedResp.CurrentLocation.Latitude = null.Float64From(33.75)
		expectedResp.CurrentLocation.Longitude = null.Float64From(-117.91)
		expectedResp.PowerTrain.Range = null.Float64From(187.79)
		expectedResp.Chasis.Axle.Row2.Wheel.Left.Tire.Pressure = null.Float64From(244)
		var e response2.Device
		e.Status = append(e.Status, expectedResp)
		deviceStatusSvc.EXPECT().PrepareDeviceStatusInformationV2(gomock.Any(), gomock.Any(), dd[0].DeviceDefinitionId, nil, []int64{1, 3, 4}).Times(1).
			Return(e)
		request := test.BuildRequest("GET", "/v2/user/device-data/"+udID+"/status", "")
		response, _ := app.Test(request)
		body, _ := io.ReadAll(response.Body)
		...
		assert.Equal(t, 187.79, snapshot.Status[0].PowerTrain.Range.Float64)
		assert.Equal(t, false, snapshot.Status[0].PowerTrain.TractionBattery.Charging.IsCharging.Bool)
		assert.Equal(t, 244.0, snapshot.Status[0].Chasis.Axle.Row2.Wheel.Left.Tire.Pressure.Float64)
		assert.Equal(t, 195677.59375, snapshot.Status[0].PowerTrain.Transmission.TravelledDistance.Float64)
		assert.Equal(t, 33.75, snapshot.Status[0].CurrentLocation.Latitude.Float64, "expected autopi latitude")
		assert.Equal(t, -117.91, snapshot.Status[0].CurrentLocation.Longitude.Float64, "expected autopi longitude")
