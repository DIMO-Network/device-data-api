...
func (o *VehicleSignalsJob) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(vehicleSignalsJobAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(vehicleSignalsJobPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert vehicle_signals_job, could not build conflict column list")
			}

			conflict = make([]string, len(vehicleSignalsJobPrimaryKeyColumns))
			copy(conflict, vehicleSignalsJobPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_data_api\".\"vehicle_signals_job\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the VehicleSignalsJob row exists.
func (o *VehicleSignalsJob) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return VehicleSignalsJobExists(ctx, exec, o.VehicleSignalsJobID)
}
