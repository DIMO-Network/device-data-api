...
APPS = device-data-api
all: $(APPS)

install: $(APPS)
	@mkdir -p bin
	@cp $(PATHINSTBIN)/device-data-api ./bin/
...
docker: deps
	@docker build -f ./resources/docker/Dockerfile . -t dimozone/device-data-api:$(VER_CUT)
	@docker tag dimozone/device-data-api:$(VER_CUT) dimozone/device-data-api:latest

docker-cgo: deps
	@docker build -f ./resources/docker/Dockerfile.cgo . -t dimozone/device-data-api:$(VER_CUT)-cgo
	@docker tag dimozone/device-data-api:$(VER_CUT)-cgo dimozone/device-data-api:latest-cgo
...
test: $(APPS)
	...
	@$(PATHINSTBIN)/device-data-api test ./config/test/...
